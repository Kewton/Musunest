...
> **production（アカウント②）は Free のまま**（`/api/*` は 404 で、動いているのは `/healthz` だけ。M2 でアプリを公開するときに再判断する）。
...
### 🧑 P-7 の後継の線（**提案。所有者の決定待ち**）

10 ms の壁が無くなったので、「重すぎる」を判定する線が無くなった。**実測の 60.60 ms をそのままにしない**ために、次の 2 本を提案する。

| 案 | 線 | 触れたらすること |
|---|---|---|
| **性能** | staging の `/api` で、落ち着いた状態の **Worker 単体の最大が 200 ms を超えた** | 課金を上げるのではなく、**作りに手を入れる**（集計の行数の上限・ページング・保存する計算値）。M1.5 の「静的チェックの上限値」と同じ議論 |
| **費用** | アカウント①の**月額が $10 を超えた** | 原因の経路を特定する。放置しない |

**上げる先がもう無い**（次は WfP で、これは L3 の話）。だから後継の線は「課金を上げる合図」ではなく、**「設計に手を入れる合図」**になる。
