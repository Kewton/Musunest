...
| R-8 | orchestrate の実行契約の制約（goal は 8000 文字まで。ソース約 30 本が目安。`.tf`・`.gitignore`・ディレクトリの移動は scope に載らない）に抵触する | 大きすぎる Issue を自動 dispatch できない | §10.1 の基準で Issue を分割する。載らない変更は監督側が担当する |
...
- 1 Issue あたり 1 パッケージ前後、ソースコード 30 本程度までにする。orchestrate の実行契約は goal が 8000 文字までで、`.tf`・`.gitignore`・ディレクトリの移動は scope に載らない。載らない変更は監督側（人と Claude）が担当する
