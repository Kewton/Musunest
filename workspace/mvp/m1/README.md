...
- **2026-09-17 の実測**（Issue #111）：結果と判定は **[`measurements.md`](./measurements.md)** にある。**P-7 に触れた**（data-api の単体の最大が、20 件で 7.29〜12.99 ms・200 件で 30.25〜60.60 ms）。P-1 は触れていない。**管理を通じて窓口へ返し、Workers Paid への変更は人が行う**
  - 静的チェックの上限値は、この実測から**根拠つきの提案**までとし、**確定は M1.5**（2026-09-16 窓口が決定。Q18-10。`03` §5.1・§6）
...
| R-3 **（顕在化・対応済み）** | 処理時間が CPU 10 ms の上限を超える（§7.2） | 利用者の画面がエラーになる。M0 の実測で、合算なら余裕は 0.64 ms しかない | **2026-09-18 完了。** M1.2 で実測し（#111・`measurements.md`）、**data-api 単体の最大 60.60 ms** で P-7 に抵触。宣言どおり議論せず**アカウント①を Workers Paid（$5＋従量）へ上げた**（`../m0/06-plan-and-limits.md` §5・§8.1）。**構成は変えていない。** 残る宿題：10 ms を超えても `exceededResources` が 0 件である理由（F-1）と、後継の線（`06` §5） |
