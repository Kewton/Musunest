...
> **Contract piece: rules.** Part of the AppSpec canonical source (`community.app-spec/v0.2`).
...
| **R-TYPE-12** An `action.when` expression must be `boolean`. | `set actions.editExpense.when = kind == "food" ⇒ OK` | `set actions.editExpense.when = description ⇒ LOGIC_ACTION_WHEN_NOT_BOOLEAN` | `set actions.editExpense.when = kind != "food" ⇒ OK` |
...
| Rule | Positive (accept → expected) | Negative (reject → expected) | Boundary (edge → expected) |
|---|---|---|---|
| **R-NAME-01** A bare name resolves to a field or computed of the **same** entity. | `set computed.headcount.expression = amount + paidAmount ⇒ OK` | `set computed.headcount.expression = ammount ⇒ LOGIC_REFERENCE_NOT_FOUND` | a name that is a field of **another** entity is not found: `set computed.headcount.expression = name ⇒ LOGIC_REFERENCE_NOT_FOUND` |
| **R-NAME-02** `entity.field` (member access) is **not writable**. | `set computed.headcount.expression = paidAmount + 1 ⇒ OK` (the same reference without a dot) | `set computed.headcount.expression = expense.amount ⇒ LOGIC_REFERENCE_OUT_OF_ENTITY` | when the left side is not a declared entity: `set computed.headcount.expression = nosuch.x ⇒ LOGIC_REFERENCE_NOT_FOUND` |
