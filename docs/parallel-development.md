...
### 6.5 依頼とセッションで踏んだこと（2026-09-24〜26）
...
| **ワーカーが「skill と契約が push を禁じている」と push を断った**（#210） | 管理が「この push と PR の作成は §6.2 の正規の流れで、管理からの指示が許可である」と送る。依頼文の push の指示に、最初からこの 1 文を入れる |
| **管理が、ワーカーの読み替えの申告を「窓口の判断待ち」としながら先に merge した**（#233） | 依頼文に「**読み替えの申告があれば、merge せずに窓口へ返す**」と書く。§7.4 の「迂回の自己申告は止める合図」と同じ扱いにする |
| `git branch --merged` が squash merge を見抜けない | worktree の片付けは `cmate-worktree-cleanup`（merge された PR・tip・merge commit・差分の一致の 4 条件）で判定する |
| 窓口は tmux のセッションを止められない（権限で拒否される） | 人に頼む。止めてほしいセッションは引き継ぎの README に「git の外」として残す |
...
- **planner が拾うのは、知っている拡張子のファイルだけである。** `.ebnf` を宣言したら scope に入らなかった（#212）。置くファイルは planner が知っている拡張子（`.md` など）にする
- **`/` の無いファイル名も、地の文に書くと scope に入る**（#211。`app.spec.yaml` と書いた）。短いファイル名も言葉に置き換える
- **依存が変わったあとは、`pnpm build` の前に `pnpm install` を打つ**（打たないと build が落ちる）。`spec:check` は誤りが 0 件のとき何も出さずに終わるので、記録には終了コードを書く
