...
import { cloudflareGatewayRelay } from "./cloudflare.js";
import type { HostEnv } from "./cloudflare.js";

/** gateway へのリクエストの origin（src/worker/cloudflare.ts の GATEWAY_ORIGIN）。host 名は解決に使われない */
...
function binding(respond: (request: Request) => Response = () => Response.json({ ok: true })): {
  ...
  return { calls, env: { GATEWAY: fetcher, ENVIRONMENT: "dev", GIT_SHA: "test" } satisfies HostEnv };
}
...
describe("cloudflareGatewayRelay（/api/* の宛先）", () => {
  it("宛先の origin を固定の内部 origin に置き換える", async () => {
    const { env, calls } = binding();
    await cloudflareGatewayRelay(env)(new Request("https://musunest-dev-host.example/api/instances/i/spec"));

    expect(calls).toHaveLength(1);
    const sent = new URL(sentTo(calls).url);
    expect(sent.origin).toBe(GATEWAY_ORIGIN);
    expect(sent.pathname).toBe("/api/instances/i/spec");
  });

  it("利用者の URL の origin では宛先を選べない（別のホスト名でも binding の宛先へ行く）", async () => {
    const { env, calls } = binding();
    await cloudflareGatewayRelay(env)(new Request("https://evil.example/api/instances/i/spec"));

    expect(new URL(sentTo(calls).url).origin).toBe(GATEWAY_ORIGIN);
    expect(sentTo(calls).url).not.toContain("evil.example");
  });

  it("query をそのまま運ぶ（query に URL を書いても宛先は変わらない）", async () => {
    const { env, calls } = binding();
    await cloudflareGatewayRelay(env)(
      new Request("https://musunest-dev-host.example/api/instances/i/views/v?page=2&target=https%3A%2F%2Fevil.example"),
    );
  ...
  it("method・ヘッダ・body をそのまま運ぶ（X-Musunest-Probe を載せ足さない）", async () => {
    const { env, calls } = binding();
    const body = JSON.stringify({ description: "夕食", amount: 6600 });
    await cloudflareGatewayRelay(env)(
      new Request("https://musunest-dev-host.example/api/instances/i/actions/addExpense", {
        method: "POST",
    ...
    expect(sent.headers.get("x-musunest-probe")).toBeNull();
  ...
  it("下流の応答をそのまま返す（status・content-type・body を組み替えない）", async () => {
    ...
    const res = await cloudflareGatewayRelay(env)(new Request("https://musunest-dev-host.example/api/instances/i/spec"));
  ...
  it("binding はリクエスト1つにつき1回だけ呼ばれる", async () => {
    const { env, calls } = binding();
    const relay = cloudflareGatewayRelay(env);
    await relay(new Request("https://musunest-dev-host.example/api/instances/i/spec"));
    await relay(new Request("https://musunest-dev-host.example/api/instances/i/views/v"));
